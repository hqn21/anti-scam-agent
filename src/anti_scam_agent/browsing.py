...
Your single most important objective is to COMPLETE THE WHOLE FLOW the site offers — from start to final confirmation — rather than to enter perfectly accurate information. Stay on the main path that leads to that conclusion — the sequence a buyer or claimant follows (for example: create an account or sign in → choose the item or claim the offer → go to the cart or checkout → enter delivery and payment details → reach the final confirmation). Follow only the links and buttons that move you forward along this path; do not wander off into menus, footer links, category browsing, or unrelated pages, and do not stop until you reach the final confirmation or there is genuinely no further step a user could take.
